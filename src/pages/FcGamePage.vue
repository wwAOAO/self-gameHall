...
import { fcGames, loadFcGame, type FcGame } from '@/data/fcGames';
...
const selectedGame = ref<FcGame>(fcGames[0]);
...
function resetPlayer() {
    ...
    const oldScript = document.querySelector('script[data-gamehall-emulatorjs="fc"]');
    oldScript?.remove();
    if (playerRef.value) {
        playerRef.value.innerHTML = '';
        const mount = document.createElement('div');
        mount.id = 'fc-emulator-player';
...
async function startEmulator(gameUrl: string, label: string) {
    ...
    try {
        ...
        window.EJS_player = '#fc-emulator-player';
        window.EJS_core = 'nes';
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            script.dataset.gamehallEmulatorjs = 'fc';
...
async function loadBundledRom() {
    const romUrl = game.value.loadUrl || game.value.romPath;
    if (!romUrl) {
    ...
    await startEmulator(romUrl, game.value.fileName || game.value.name);
}
...
onMounted(async () => {
    selectedGame.value = await loadFcGame(route.params.id);
    resetPlayer();
    ...
    <div class="min-h-screen bg-[#0f1218] text-white">
        <main class="mx-auto flex min-h-screen w-full max-w-[1260px] flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8">
                ...
                <aside class="side-panel">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm leading-6 text-slate-300">
                            EmulatorJS NES：优先使用 fceumm，可在模拟器菜单中切换 nestopia。
                        </p>
                    </div>
            ...
            <input
                ...
                class="hidden"
                type="file"
                accept=".nes,.zip,application/zip,application/octet-stream"
...
.screen-shell {
    ...
    background:
        ...
        radial-gradient(circle at 50% 10%, rgba(34, 211, 238, 0.11), transparent 32%), #05070b;
}
...
.screen-mount :deep(#fc-emulator-player) {
    width: 100%;
    height: 100%;
    min-height: 360px;
}
