...
import { gbaGames as configuredGbaGames, loadGbaGames, type GbaGame } from '@/data/gbaGames';

const router = useRouter();
const games = ref<GbaGame[]>(configuredGbaGames);
...
const filteredGames = computed(() => {
    ...
    return games.value.filter(game => {
        const searchableText = [game.name, game.desc, game.tag, game.format, game.fileName ?? '', game.romPath ?? '']
            .join(' ')
            .toLocaleLowerCase();
...
onMounted(async () => {
    games.value = await loadGbaGames();
});
</script>
        ...
        <main class="mx-auto flex min-h-screen w-full max-w-[1180px] flex-col gap-5 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
            ...
            >
                <div>
                    ...
                    <div class="mt-5 flex items-center gap-3">
                        <div
                            class="grid h-12 w-12 place-items-center rounded-lg bg-violet-400/12 text-violet-100 ring-1 ring-violet-300/20"
                        >
                            <Gamepad2 class="h-7 w-7" />
                        </div>
                        <div>
                            <h1 class="text-2xl font-bold tracking-normal sm:text-3xl">GBA 游戏</h1>
                            <p class="mt-1 text-sm leading-6 text-slate-400">
                                加载 .gba 卡带，支持键盘控制、全屏、即时状态和电池存档。
                            </p>
                        </div>
                    </div>
                </div>
            ...
            >
                <label class="relative block w-full sm:max-w-md">
                    ...
                    <input
                        v-model="searchText"
                        class="h-11 w-full rounded-md border border-white/10 bg-black/20 pl-10 pr-10 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-violet-300/40 focus:bg-black/30"
                        type="text"
                ...
                >
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="grid h-12 w-12 place-items-center rounded-lg bg-black/20 ring-1 ring-white/10">
                            <FolderOpen v-if="game.id === 'local'" class="h-6 w-6 text-violet-200" />
                    ...
                    <div class="mt-4 flex items-center justify-between border-t border-white/10 pt-3">
                        <span class="truncate pr-3 text-xs text-slate-500">{{ game.fileName || game.format }}</span>
                        <span
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-400">
                        换个关键词试试，或确认 .gba 文件已经放在 public/roms/gba。
                    </p>
                </div>
            </section>
            ...
            >
                ...
                <p>
                    ROM 和 BIOS 文件请使用你拥有授权的版本。固定卡带放在
                    <span class="font-mono text-emerald-100">public/roms/gba/</span>，列表会自动读取生成的清单。
                </p>
            </aside>
        </main>
    </div>
