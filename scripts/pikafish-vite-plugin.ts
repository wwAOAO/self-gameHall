...
class PikafishUci {
            ...
            const timeout = setTimeout(
                () => {
                    ...
                    try {
                        ...
                        // Ignore a dead engine; the caller will fall back to the local AI.
                    }
                    resolve(null);
                },
    ...
    private ensureReady(): Promise<void> {
        ...
        this.readyPromise = new Promise((resolve, reject) => {
            ...
            const startupTimeout = setTimeout(() => reject(new Error('Pikafish startup timed out')), 5000);
...
function findEnginePath(root: string): string | null {
    const enginePaths = [
        process.env.PIKAFISH_EXECUTABLE,
        path.resolve(root, 'engines', 'pikafish', process.platform === 'win32' ? 'pikafish.exe' : 'pikafish'),
...
function attachPikafishApi(
    root: string,
    middlewares: { use: (path: string, handler: import('node:http').RequestListener) => void },
) {
    const enginePath = findEnginePath(root);
    const engine = enginePath ? new PikafishUci(enginePath) : null;

    middlewares.use('/api/pikafish/status', (_req, res) => {
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ available: !!enginePath, enginePath }));
    });

    middlewares.use('/api/pikafish/bestmove', async (req, res) => {
        res.setHeader('Content-Type', 'application/json');
        ...
        if (!engine) {
            res.statusCode = 503;
            res.end(JSON.stringify({ ok: false, error: 'Pikafish executable not found', enginePath }));
            return;
        }
...
export function pikafishPlugin(): Plugin {
    let root = process.cwd();

    return {
        name: 'gamehall-pikafish',
        configResolved(config) {
            root = config.root;
        },
        configureServer(server) {
            const dispose = attachPikafishApi(root, server.middlewares);
